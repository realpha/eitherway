...
import type { IsExact, Has } from "../../dev_deps.ts";
...
Deno.test("eitherway::Task", async (t) => {
  ...
  await t.step("Task<T, E> -> Instance Methods", async (t) => {
    await t.step("Task<T, E> -> Map Methods", async (t) => {
      ...
      await t.step(".trip() -> returns Ok value as is if tripFn succeeds", async () => {
        const success: Task<number, TypeError> = Task.succeed(42);
        const tripFn = (n: number): Result<Empty, RangeError> => n % 2 === 0 ? Ok.empty() : Err(RangeError());

        const okTask = success.trip(tripFn);

        assertType<IsExact<typeof okTask, Task<number, TypeError | RangeError>>>(true);
      ...
      await t.step(".trip() -> derailts the successful Task if tripFn succeeds", async () => {
        const re = RangeError("Cannot do that");
        const success: Task<number, TypeError> = Task.succeed(41);
        const tripFn = (n: number): Result<Empty, RangeError> => n % 2 === 0 ? Ok.empty() : Err(re);
      ...
      await t.step(".trip() -> is a no-op in case of Err", async () => {
        ...
        assertType<IsExact<typeof errTask, Task<number, TypeError | RangeError>>>(true);
    ...
    await t.step("Task<T, E> -> Transformation Methods", async (t) => {
      await t.step(".iter() -> returns an AsyncIteratable<T> in case of success", async () => {
        ...
        assertType<IsExact<typeof okIter, AsyncIterableIterator<string>>>(true);
        assertStrictEquals(count, 1);
        assertStrictEquals(value, "foo");
      });

      await t.step(".iter() -> returns an empty AsyncIteratable<T> in case of failure", async () => {
        ...
        assertType<IsExact<typeof errIter, AsyncIterableIterator<string>>>(true);
    ...
    await t.step("Task<T, E> -> JS well-known Symbols & Methods", async (t) => {
      await t.step("[Symbol.asyncIterator]() -> delegates to the underlying implementation in case of success", async () => {
        ...
        assertType<IsExact<typeof okIter, AsyncIterableIterator<number>>>(true);
        assertStrictEquals(count, 1);
        assertStrictEquals(value, 42);
      });

      await t.step("[Symbol.asyncIterator]() -> returns an empty AsyncIterable<never> if success value doesn't implement async iterator protocol", async () => {
        ...
        assertType<IsExact<typeof okIter, AsyncIterableIterator<never>>>(true);
        assertStrictEquals(count, 0);
        assertStrictEquals(value, undefined);
      });

      await t.step("[Symbol.asyncIterator]() -> returns an empty AsyncIterable<never> in case of failure", async () => {
        ...
        assertType<IsExact<typeof errIter, AsyncIterableIterator<never>>>(true);
        assertStrictEquals(count, 0);
        assertStrictEquals(value, undefined);
      });
    });
